...
public class PhonebookController {
	...
	public String write2(@RequestParam(value = "name") String name, 
		...
		// 리스트로 리다이렉트
		return "redirect:/phonebook5/phone/list"; // 리다이렉트는 고치면 안됨(feat.View Resolver 설정)
	}
	...
	public String list(Model model) {
		...
		// 메모리에 올려줘 (제3자에게 주입=> 제어의 역전.따라서 new 라고 객체 생성 필요X)
		// PhonebookService phonebookService = new PhonebookService();@Service 덕분에 자동연결
		// 됨.
	...
	public String delete(@RequestParam("no") int no) {
		...
		/*
		 * 바로 dao 쓰는 코드 있으면 안됨. Service 쓰고 있잖아 dao를 메모리에 올린다 PhonebookDao phonebookDao =
		 * new PhonebookDao(); //메소드(기능처리) phonebookDao.personDelete(no);
		 */
